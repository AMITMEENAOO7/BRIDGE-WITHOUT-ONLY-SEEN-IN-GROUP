...
const Sidebar = () => {
  ...
  useEffect(() => {
    if (socketConnection) {
      ...
      socketConnection.on('updateUnseenMessageCount', ({ userId, unseenCount }) => {
                ...
                <div className='text-slate-500 text-xs flex items-center gap-1'>
                  <div className='flex items-center gap-1'>
                    {conv?.lastMsg?.imageUrl && (
                      <div className='flex items-center gap-1'>
                        <span><FaImage /></span>
                        {!conv?.lastMsg?.translatedText && <span>Image</span>}
                      </div>
                    )}
                    {conv?.lastMsg?.videoUrl && (
                      <div className='flex items-center gap-1'>
                        <span><FaVideo /></span>
                        {!conv?.lastMsg?.translatedText && <span>Video</span>}
                      </div>
                    )}
                  </div>
                  <p className='text-ellipsis line-clamp-1'>{conv?.lastMsg?.translatedText}</p>
                </div>
              </div>
              {Boolean(unseenCounts[conv?.userDetails?._id]) && (
                <p className='text-xs w-6 h-6 flex justify-center items-center ml-auto p-1 bg-primary text-white font-semibold rounded-full'>{unseenCounts[conv?.userDetails?._id]}</p>
              )}
            </NavLink>
          ))}
        </div>
